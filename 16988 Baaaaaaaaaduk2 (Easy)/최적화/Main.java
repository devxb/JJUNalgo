...
public class Main{
	...
	private void solve(){
		...
		int killPieceCount = this.playBaduk(0, 0, board);
	...
	private int playBaduk(int oneMatrixPos, int killCount, int[][] board){
		...
		for(int nowY = 0; nowY < board.length; nowY++){
			for(int nowX = 0; nowX < board[nowY].length; nowX++){
				int nowOneMatrixPos = this.calcOneMatrixPos(nowY, nowX, board[nowY].length);
				if(oneMatrixPos > nowOneMatrixPos || !this.isPutable(board[nowY][nowX])) continue;
				board[nowY][nowX] = 1;
				ans = Math.max(ans, this.playBaduk(nowOneMatrixPos, killCount+1, board));
				board[nowY][nowX] = 0;
			}
		}
		return ans;
	...
	private int calcOneMatrixPos(int y, int x, int width){
		return y * width + x;
	}
